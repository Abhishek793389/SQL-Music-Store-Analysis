...
select round(total , 2) as total_invoice from invoice
order by total desc
limit 3;
...
select billing_city as city , round(sum(total),3) as invoice_total
from invoice
...
-- Q).Return all the track names that have a song length longer than the average song length. Return the Name and Milliseconds for each track. Order by the song length with the longest songs listed first.for

select name, milliseconds
from track
where milliseconds > (select avg(milliseconds) as length_of_songs from track)
